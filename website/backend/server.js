...
const corsOptions = {
  origin: "http://192.168.111.129:5173", // or the specific origin you want to allow
...
app.get("/api/fetch_grade_with_pc", async (req, res) => {
  ...
  try {
    pool = await sql.connect(config);
    const query = `SELECT id as code,grade,FSC_Y_N,Grade as name,Profit_Centre as profitCenter FROM profit_center where status = 1 and FSC_Y_N = '${fsc}'`;
    const result = await pool.request().query(query);
