...
async function getLowestPaymentTerm(req, res) {
  try {
    ...
    logger.info(
      `Fetching lowest payment term details with customers: ${customers}, consignees: ${consignees}, endUses: ${endUses}`
    );
    ...
    logger.info(
      `Lowest payment term details fetched successfully: ${JSON.stringify(
        lowestPaymentTerm
      )}`
    );
    res.json({
      lowestPaymentTerm: lowestPaymentTerm || "C030 - Payment within 30 days",
    });
  } catch (error) {
    logger.error(
      `Error fetching lowest payment term details: ${error.message}`
    );
    res.status(500).send({
...
async function getProfitCenter(req, res) {
  try {
    ...
    logger.info(
      `Profit center details fetched successfully: ${JSON.stringify(
        result.recordset
      )}`
    );
